...
const HTTP_URL = "https://smart-spoon-ai.onrender.com"; // Added for Mode Switching
// ============================================================================
...
export default function App() {
  ...
  // ============================================================================
  // GLOBAL TARGET MODE STATE (Apple, Milk, Water)
  // ============================================================================
  ...
  // ============================================================================
  // HANDLE MODE SWITCHER VIA FASTAPI BACKEND
  // ============================================================================
  const handleModeSwitch = async (modeId) => {
    ...
    try {
      await fetch(`${HTTP_URL}/set_mode`, {
        ...
        // Fallback safety if the backend only has 'apple' or 'milk'
  ...
  const handleChatSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      let reply = `System operational. Current sample: ${hero.adulteration_type} with a safety rating of ${firstNumber(primary["1_safety_score"], 0)}/100. Target profile is ${targetProfile.toUpperCase()}.`;
  ...
  // Variables for the UI and Health Matrix
  const liveFreq = meta.processed_mean_hz || meta.excitation_frequency_hz || 0;
  ...
  // ============================================================================
  // RESTORED 8-POINT CONSUMER HEALTH & SAFETY MATRIX
  // ============================================================================
      ...
      <header className={`sticky top-0 z-40 border-b backdrop-blur-md transition-colors ${
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap justify-center sm:justify-end">
            ...
            {/* ======================================================================= */}
            {/* NEW TARGET MODE SWITCHER IN THE HEADER                                  */}
            {/* ======================================================================= */}
            <div className="flex bg-slate-900 border border-slate-700 rounded-lg p-1 shadow-inner mr-2">
              {[
                { id: "apple", label: "🍏 Apple Extract" },
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-[10px] sm:text-xs font-bold uppercase tracking-wider transition-all ${
                    targetProfile === mode.id
            ...
            >
              ...
              <span className="hidden sm:inline">{voiceActive ? (isSpeaking ? "Speaking…" : "Voice ON") : "Voice OFF"}</span>
            </button>
            <InstallApp />
        ...
        {activeTab === "telemetry" && (
          <div className="space-y-8">
            ...
            <section>
              ...
              {/* RESTORED 8-POINT CONSUMER HEALTH & SAFETY MATRIX                        */}
              {/* ======================================================================= */}
              <div className="mt-6 rounded-2xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-sm p-6 relative overflow-hidden">
                <div className="flex items-center gap-3 mb-6">
                  <ClipboardCheck className="w-5 h-5 text-cyan-400" />
                  <h3 className="text-sm font-bold uppercase tracking-[0.15em] text-slate-300">Consumer Health & Safety Matrix</h3>
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {consumerSafetyData.map((item, idx) => (
                    <div key={idx} className={`p-4 rounded-xl border flex flex-col justify-between h-24 relative overflow-hidden transition-colors duration-500 ${
                      isAwaiting 
                        ? "border-slate-800 bg-slate-900/50" 
                        : item.isSafe 
                          ? "border-emerald-500/20 bg-emerald-500/10" 
                          : "border-rose-500/30 bg-rose-500/10 shadow-[inset_0_0_15px_rgba(244,63,94,0.1)]"
                    }`}>
                      {!item.isSafe && !isAwaiting && (
                        <AlertTriangle className="absolute -bottom-4 -right-4 w-16 h-16 text-rose-500/10 -z-10" />
                      )}
                      <div className="text-[10px] font-bold uppercase text-slate-400 tracking-wider z-10 leading-tight">
                        {item.label}
                      </div>
                      <div className={`text-xs md:text-sm font-black tracking-tight z-10 ${
                        isAwaiting 
                          ? "text-slate-500" 
                          : item.isSafe 
                            ? "text-emerald-400" 
